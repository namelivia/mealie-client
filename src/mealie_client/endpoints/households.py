...
class HouseholdsManager:
    ...
    async def get_all(
        ...
        return [
            HouseholdSummary.from_dict(household_data)
    ...
    async def create(self, household: HouseholdCreateRequest) -> Household:
        ...
        if hasattr(household, "to_dict"):
            ...
            household_data = cast(Dict[str, Any], household)

        response = await self.client.post(
            "admin/households", json_data=clean_dict(household_data)
        )
        return Household.from_dict(response)

    async def update(
        self, household_id: str, household: HouseholdUpdateRequest
    ) -> Household:
        """
        ...
        response = await self.client.put(
            f"admin/households/{household_id}", json_data=clean_dict(household_data)
        )
        return Household.from_dict(response)
